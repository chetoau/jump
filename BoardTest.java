...
public class BoardTest {
    ...
    public void testCopy() {
        Board newB = new Board(4);
        newB.addSpot(RED, 1, 4);
        newB.addSpot(BLUE, 1, 2);
        assertEquals(1, newB.numOfSide(RED));
        assertEquals(1, newB.numOfSide(BLUE));
        newB.addSpot(RED, 2, 1);
        newB.addSpot(BLUE, 3, 2);
        Board newC = new Board(4);
        newC.copy(newB);
        checkBoard("C board", newC, 1, 4, 2, RED,
                1, 2, 2, BLUE, 2, 1, 2, RED, 3, 2, 2, BLUE);
        assertNotSame(newC, newB);
        assertEquals("newB and newC have the same contents", newB, newC);
    ...
    public void testUndo() {
        ...
        newB.addSpot(RED, 2, 1);
        newB.addSpot(BLUE, 3, 2);
        checkBoard("before undo", newB, 1, 4, 2, RED, 1, 2, 2,
                BLUE, 2, 1, 2, RED, 3, 2, 2, BLUE);
        newB.undo();
        checkBoard("after 1st undo", newB, 1, 4, 2, RED,
                1, 2, 2, BLUE, 2, 1, 2, RED);
        newB.undo();
        checkBoard("after 2nd undo", newB, 1, 4, 2, RED,
                1, 2, 2, BLUE);
        newB.undo();
        checkBoard("after 3rd undo", newB, 1, 4, 2, RED);
        newB.undo();
        checkBoard("after last undo", newB);
    ...
    public void testGetWinner() {
        ...
        B.set(2, 1, 2, RED);
        B.set(2, 2, 2, RED);
        assertEquals("Winner is", RED, B.getWinner());
    }

    @Test
    public void testSampleBoard1() {
        Board newB = new Board(3);
        checkBoard("#0", newB);
        newB.addSpot(RED, 1, 2);
        assertEquals("checks total number of spots", 10, newB.numPieces());
    }
